...
extern volatile uint32_t USB_DEV_count;
extern volatile uint32_t USB_FS_count;
extern uint32_t USB_FS_counter;
...
void MONITOR_Initialize(void) {
    ...
    monitorData.led_red_state = 0;    
    monitorData.led_red_count = 0;
    monitorData.led_green_state = 0;
    monitorData.led_green_count = 0;
...
void MONITOR_Display_Status(void) {
    ...
    sprintf(str,
            ...
            VT100_CLR_LINE
            "UART Write:%d TCP Write:%d USB_Dev:%d USB_Fs:%d"
    ...
    //    Console_USB_CDC_Write(0, str, strlen(str));
}
